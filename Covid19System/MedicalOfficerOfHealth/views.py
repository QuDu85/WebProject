...
from NormalUser.forms import MedicalDeclarationForm
    ...
    if request.method == 'POST' and 'vaccine_1st' in request.POST:
        ...
        for index, row in covid_certificate.iterrows():
            ...
            try:
                ...
                if Exception==CovidCertificate.DoesNotExist:
                    error = row['fullname'] + ' với số CMT/CCCD: ' + row['national_id'] + ' chưa có dữ liệu tiêm mũi 1 trong hệ thống!'
                messages.error(request, error)
...
def search(request):
    if not request.method=='POST':
        ...
        records = CovidCertificate.objects.filter(pk__icontains=target) | CovidCertificate.objects.filter(fullname__icontains=target)
...
def trace(request):
    if not request.method=='POST':
        ...
        records = MedicalDeclaration.objects.all()
        target = ''
    else:
        target = request.POST['target']
        tarUpper = target.upper()
        tarLower = target.lower()
        province = request.POST['province'] 
        arrival_province = request.POST['arrival_province']
        form = MedicalDeclarationForm(initial={'province':province,'arrival_province':arrival_province})
        if province and arrival_province:
            records = MedicalDeclaration.objects.filter(full_name__icontains=tarUpper, province=province, arrival_province=arrival_province) | \
                    MedicalDeclaration.objects.filter(full_name__icontains=tarLower, province=province, arrival_province=arrival_province) | \
                    MedicalDeclaration.objects.filter(id_card__icontains=target, province=province, arrival_province=arrival_province)
        elif province:
            records = MedicalDeclaration.objects.filter(full_name__icontains=tarUpper, province=province) | \
                    MedicalDeclaration.objects.filter(full_name__icontains=tarLower, province=province) | \
                    MedicalDeclaration.objects.filter(id_card__icontains=target, province=province)
        elif arrival_province:
            records = MedicalDeclaration.objects.filter(full_name__icontains=tarUpper, arrival_province=arrival_province) | \
                    MedicalDeclaration.objects.filter(full_name__icontains=tarLower, arrival_province=arrival_province) | \
                    MedicalDeclaration.objects.filter(id_card__icontains=target, arrival_province=arrival_province)
        else:
            records = MedicalDeclaration.objects.filter(full_name__icontains=tarUpper) | \
                    MedicalDeclaration.objects.filter(full_name__icontains=tarLower) | \
                    MedicalDeclaration.objects.filter(id_card__icontains=target)
    paginator = Paginator(records, 25) # Show 25 records per page.

    page_number = request.GET.get('page',1)
    page_obj = paginator.get_page(page_number)
    return render(request, 'trace_MOH.html', {'page':page_obj, 'target':target, 'form':form})

@login_required(login_url='/MOH')
@user_passes_test(MOH_check, login_url='/')
def show_kbyt_detail_MOH(request, id):
