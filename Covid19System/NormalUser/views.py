...
@user_passes_test(normal_check, login_url='/')
def home_normal_user(request):
    ...
    return render(request, 'home_normal_user.html', {'svg': svg})
...
def edit_profile(request):
    if request.user.is_authenticated and request.user.is_normal_user is True:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid() :
                form.save()
                messages.success(request, 'Cập nhật thông tin!')
...
def medical_declaration(request):
    if request.user.is_authenticated and request.user.is_normal_user is True:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid() :
                form.save()
                messages.success(request, 'Khai báo y tế thành công!')
